...
{
	//extract just the file name from the path (cut out the default path and the extension)
	Name = path.substr(SpriteFontManager::defaultPathLen, path.length() - SpriteFontManager::defaultPathLen);
	...
	while (S_OK == reader->Read(&nodeType))
	{
		...
		case XmlNodeType::XmlNodeType_EndElement:
		{
			...
			if (wcscmp(stringValue, L"character") == 0)
			{
				...
				ImageManager::loadImage(Name + ": Key " + to_string(key), FontTexture, new Rect((float)x, (float)y, (float)w, (float)h));
				Glyph* newGlyph = new Glyph(Name + ": Key " + to_string(key));
				newGlyph->setCenter((float)x, (float)y);
				fontmap.insert(std::pair<Key, Glyph*>(key, newGlyph));
			}
...
{
	//(int)c gives its ASCII value
	Key keyToFind = (int)c;
	//Make sure the glyph exists
	FontMap::iterator it = fontmap.find(keyToFind);
	if (it == fontmap.end())
	{
		throw std::runtime_error("Image " + to_string(keyToFind) + " not found!");
	}
	return (*it).second;
}
